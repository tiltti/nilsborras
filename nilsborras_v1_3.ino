...
// TODO: Set to something reasonable after debugging
...
void setup(){
  ...
  Serial.print("Determining ambient light status...  ");
...
void loop(){
  ...
  // check if it's dark or not to lit the stairs   
  lightStatus = checkLights(lightStatus);
...
boolean checkLights(boolean existingLightStatus) {
  ...
  if(sensorValue<lightTreshold) {
    ...
    if(!existingLightStatus) {
      ...
      Serial.print(" sec to TRUE. Value: "); 
    ...
    if(existingLightStatus) {
      ...
      Serial.print("Lightning conditions changed at ");
      Serial.print(millis()/1000);
      Serial.print(" sec to FALSE. Value: "); 
      Serial.println(sensorValue, DEC);
      beep(20,10,1);
      delay(200);
    }
